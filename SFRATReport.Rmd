...
tryCatch({                                  #Data conversion depending on the type of the input data
  if("FN" %in% cnames && "IF" %in% cnames && "FT" %in% cnames) { #"FN" determiners the number of Failures
                                                                 #"IF" determines interfailure time vector
                                                                 #"FT" determines failure time vector
    FT <- d$FT
    IF <- d$IF
    FN <- d$FN
    ...
    FT <- d$FT
    FN <- d$FN
  } else if("T" %in% cnames && "FC" %in% cnames && "CFC" %in% cnames) { #"FC" determines failure count vector
                                                                        #"CFC" determines cumulative failure count vector 
    FC <- d$FC
    CFC <- d$CFC
...
\newpage
##Laplace Trend Test
`r if (x==1){paste0("The following figure shows the Laplace test statistic for reliability growth as a function of cumulative test time (FT). A decreasing trends indicates reliability growth, while an increasing trend indicates reliabilty deterioration. The Laplace test statistic on the y-axis corresponds to the critical values of a normal distribution. This means that if the trend falls below a specific level, then we cannot reject the null hypothesis that the failure data suggests reliability growth at a specified level of confidence. The six black dot-dash style lines correspond to the 90%, 95%, 99%, 99.9%, 99.9999%, and 99.999999% respectively. The red line is use-specified and has been set to ",percent_data_for_PSSE*100,"%. The level of confidence is a subjective choice made by the analyst. Reliability growth is desired because software reliability growth models assume curves that exhibit increasing time between failures. If reliability growth is not present than the model fitting step may fail or produce predictions that are inaccurate. Therefore, the Laplace test statistic provides an objective quantitative measure for the analyst to decide if predictions may or may not be accurate.")}`
...
`r if (x==1){paste0("The running arithmetic average plots the average of the first k times between failure as a function of the kth failure. An increasing trends indicates reliability growth, while a decreasing trend indicates reliability deterioration. This is intuitive because if the time between failures is increasing, then later failures will increase the average.")}`
...
##Cumulative failures
`r if (x==1){paste0("The following figure shows the ",paste(models_to_apply, collapse=", "), " models fit to the cumulative number of failures detected in the ",SheetName," data.")}`
...
`r if (x==1){paste0("The following figure shows the ",paste(models_to_apply, collapse=", "), " models fit to the times between failures detected in the ",SheetName," data.")}`
...
`r if (x==1){paste0("The following figure shows the ",paste(models_to_apply, collapse=", "), " models fit to the failure intensity of the ",SheetName," data.")}`

```{r, echo=FALSE}
tempFI <- 1/(FT_to_IF(hft[,which(hft==max(hft))/dim(hft)[1]]))
tempFI[is.infinite(tempFI)] <- 0
upLim <- max(max(hxyFI[,1]),max(hxyFI[,2]),max(hxyFI[,3]),max(hxyFI[,4]),max(hxyFI[,5]))
plot(
...
`r if (x==1){paste0("The following table shows inferences enabled by the models, including the time to achieve a reliability of xx (probability of zero failures for xx time units), expected number of failures in the next xx time units, and expected time to observe an additional xx failures computed for the Delayed s-Shaped (DSS), Geometric (GM), Weibull (Wei), Goel-Okumoto (GO), and Jelinski-Moranda (JM) models.")}`
